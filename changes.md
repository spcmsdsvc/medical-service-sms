...
- The existing dirty `scheduler.db`, planning journals, loose handoff, `.claude/`, `output/`, and `tmp/` were preserved and remain unstaged. The implementation is committed as `3462e63`; owner-authorized push publication will follow the explicit remote and Railway verification rules, while manual deployment and production-data access remain excluded.
