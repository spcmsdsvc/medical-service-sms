...
- The intentional pre-change focused contract failed against the unchanged source because the Page 2 cap remained 28, paste delegation was absent, and the current editor/template still used the old performance wording.

- Implemented the Calibration Report package in `static/js/app-calibration-report.js`: Page 2 Mechanical and Generator Result fields now use a 60-character exact-fit/UI cap, preserving rejection of longer or multiline values; Pages 3/4 exposure grids now accept atomic Excel/TSV rectangular paste within the focused Small/Large table and tube, including intentional blank clears and existing dirty/generated-invalidation/autosave behavior.
- Updated the Page 3/4 first performance criterion from ±6% to ±8% in the editor and canonical `static/templates/calibration-report/calibration-report-template.docx`, preserving Page 2 generator 5.2 at 6%, the above-100kVp 6kV rule, and the DOCX package structure. The template checksum is now `E2FB97EF0C9FDEDADE42478ED727A31AE10135C2671D154B11570DC1C99B00AD`.
- Bumped the Calibration Report JavaScript query to v36 and the service-worker shell/precache marker to v200, and added the published `2026-09-26-calibration-report-paste-criteria` release item. Updated the focused cache/template contracts and added runtime paste/criteria/package checks.
- Verification: the focused fail-first contract failed 1/1 against the unchanged source before implementation; afterward `tests.test_tsr_calibration_report` passed 24/24 and the combined Calibration Report/PDF/offline follow-up batch passed 55/55. Bundled Node JavaScript syntax, Jinja/Python AST, release JSON, DOCX package/content checks, and `git diff --check` passed. Full discovery used a unique external database and ran 1,332 tests with 1,311 passes, 19 unrelated baseline/stale or environment failures (older Calibration Center v195 expectation, changelog manifest synchronization, standalone LPR authorization, and purchase-order rate limiting), and 2 skips. The available DOCX renderer could not run because bundled LibreOffice `soffice.exe` was unavailable, so no render inspection was claimed. No browser/Codex UI automation, commit, push, deployment, Railway, production, database, or protected-dirty-file operation occurred.
- The owner authorized committing and pushing only this Calibration Report package to `origin/main`. The implementation, canonical DOCX, focused tests, cache/release metadata, plan, and change record were committed as `b03fc23`; protected `scheduler.db`, handoffs, `.claude/`, `output/`, `tmp/`, and unrelated files remained excluded.
...
- Full unittest discovery ran once with a disposable database: 1,331 tests with 19 unrelated existing/stale or environment failures and 2 skips, matching the prior failure areas (Calibration Center marker, changelog manifest sync, standalone LPR authorization, and purchase-order rate limiting). No browser/Codex UI, commit, push, deployment, Railway, production, or protected-file operation was performed.

- Implemented the owner-authorized TSR Autosave, Calibration Draft Indicator, and Client Groups package. Create TSR now labels each saved version that contains an active Calibration Report, keeps the existing serialized device/account draft flow with a 900 ms account-backup debounce, and mirrors Unsaved, Saving, device-saved, account-backed-up, conflict, and failure states inside the Calibration Report editor; late reports on completed TSRs remain device-only until Save Final Report.
- Added nullable `Client.group_name` storage with guarded additive startup migration, administrator add/update and non-HR `/get_clients` projection, preserved the exact HR `{id, name}` response, and kept contact-only updates from changing groups. The Clients modal now offers reusable group suggestions, group display in desktop/mobile/details, and a case-insensitive group filter; Timeline quick-add remains ungrouped.
- Added focused fail-first/source and isolated client-route tests, advanced the service-worker shell to `medical-service-pwa-offline-navigation-v198-tsr-autosave-client-groups`, bumped Calibration Report asset queries, and added the published `2026-09-26-tsr-autosave-client-groups` release entry. No browser/Codex UI, commit, push, deployment, Railway, production, or protected-dirty-file operation was performed.
- Fail-first verification against the unchanged source ran `tests.test_tsr_autosave_client_groups`: 6 tests ran with 4 failures and 2 errors, as expected before implementation.
- Post-change focused verification passed 126 tests across the TSR autosave, Calibration Report, client, dark-mode, draft-sync, sync-reliability, offline-follow-up, and contact-suggestion suites. Python AST, Jinja template parsing, JavaScript syntax, release JSON, and `git diff --check` validations also passed.
- Full unittest discovery used a unique disposable test database and ran 1,330 tests with 19 failures and 2 skips. The failures were unrelated existing/stale expectations or suite-wide test-environment effects (old Calibration Center cache marker, changelog manifest synchronization, standalone LPR authorization, and purchase-order rate limiting); no focused package suite failed.
...
- Began the separately authorized Regional Engineer Add/Edit Access package. Recorded the complete executable plan at the top of `plans.md` as **In progress** for 2026-09-26. Scope is limited to linked Cebu/BC02 and Davao/BC03 engineer Add/Edit permission for Products, Genoray, and Vieworks, focused tests, release metadata, and required records; Manila/BC01 engineers, destructive/admin-only actions, protected dirty artifacts, browser automation, commit, push, deployment, Railway, production, and database operations remain excluded.
- Implemented the shared active/non-approver Engineer branch predicate in `app.py`, accepting only linked Engineer profiles resolved to BC02/Cebu or BC03/Davao. Added narrow Product, Genoray, and Vieworks Add/Edit helpers and applied them to page flags plus the matching POST/PUT routes; Delete, CSV import, PM, Product P.O. coverage, and other administrator-only behavior remain unchanged.
- Updated `tests/test_product_vieworks_links_history.py`, `tests/test_genoray_inventory.py`, and `tests/test_vieworks_inventory.py` with Cebu/Davao positive controls, Manila/missing/unsupported/inactive denials, rendered edit gating, direct endpoint/no-mutation checks, administrator coverage, and delete/import/PM denial. The intentional fail-first run against unchanged source recorded Product 2 tests with 2 failures/4 errors, Genoray 2 tests with 1 failure/4 errors, and Vieworks 2 tests with 5 errors; the post-change focused inventory/admin/mutation batch passed 45/45.
- Added the published `2026-09-26-regional-engineer-inventory-edit` release item for admins and engineers without a service-worker bump. AST, relevant Jinja, release JSON, and `git diff --check` validations passed. Full disposable-database discovery ran 1,335 tests with 1,314 passes, 19 unrelated baseline/order-dependent failures (Calibration Center v195, changelog manifest synchronization, standalone LPR, and purchase-order rate-limit setup), and 2 skips.
- Final review confirmed only the authorized source, focused tests, release metadata, `plans.md`, and `changes.md` changed; pre-existing `scheduler.db`, handoff, `.claude/`, `output/`, and `tmp/` paths remain protected and untouched. Work is complete locally but uncommitted, unpushed, undeployed, and without Railway/production/database changes.
- Closed the regional inventory access plan in `plans.md` with the truthful executed/uncommitted outcome and the exact verification totals above.
- The owner authorized committing and pushing only the Regional Engineer Add/Edit Access package to `origin/main`. The application change, focused tests, and release metadata were committed as `c29a00e`; protected database, handoff, output, temporary, and unrelated files remained excluded.
...
- Recorded the owner's complete Editable PM cadence with future-plan rebuilding plan at the
  ...
  the edited target date while preserving earlier history. It requires transactional future-
  sibling replacement, collision rejection, linked/completed-history protection, legacy-row
  handling, an editable cadence/rebuild preview and confirmation UI, release/cache updates,
  focused fail-first tests, full non-browser verification, and protected-worktree review.
  No PM behavior, database, Railway setting, commit, push, or production state was changed
  by this approval record.
- Recorded the owner's complete Recurring PM generation and schedule matching plan at the
  top of `plans.md` with status **Approved — awaiting go-ahead**. It amends the uncommitted
  Genoray/Vieworks PM package with required Semi-Annual or Quarterly plan generation,
  deterministic month-end clamping, transactional duplicate skipping, independent visit
  management, cadence display, and schedule options limited to the visit's exact month/year,
  Client ID, PM service task, and normalized Product/equipment name. The plan also specifies
  historical-link preservation, fail-first coverage, additive schema handling, release and
  cache updates, full non-browser verification, and protected-worktree safeguards. No PM
  implementation, database, Railway, commit, push, or production behavior was changed by
  this approval record.
- Recorded the owner's complete Genoray and Vieworks PM monitoring plan at the top of
  `plans.md` with status **Approved — awaiting go-ahead**. The plan specifies separate
  administrator-only fiscal-year overviews and equipment details, manually entered exact
  target dates, validated links to existing same-client PM service schedules, derived
  completion/engineer/date and permitted file links, owner-change warnings, serial-edit
  preservation, deletion blocking, isolated tests, release metadata, service-worker bump,
  protected-worktree review, and separately gated implementation/publication. No PM code,
  database, Railway setting, or production behavior was changed by this approval record.
- Began the separately authorized Genoray and Vieworks PM monitoring implementation after
  the owner's explicit 2026-09-16 go-ahead. Preflight confirmed the approved scope, the
  existing standalone Genoray/Vieworks inventory implementation, the protected dirty
  `scheduler.db`, handoff, `.claude/`, `output/`, and `tmp/` artifacts, and the restriction
  against commit, push, deployment, production data, Railway changes, and browser/Codex UI
  automation. Source changes and isolated disposable-database verification are in progress.
- Added the isolated disposable SQLite PM monitoring test checkpoint at
  `tests/test_inventory_pm.py`. The fail-first contract and route tests intentionally run
  before the PM implementation so missing storage, routes, overview/detail data, schedule
  validation, dynamic status, serial preservation, deletion blocking, and brand isolation
  are observable without opening or modifying protected `scheduler.db`.
- Fail-first checkpoint ran with `venv\\Scripts\\python.exe -m unittest
  tests.test_inventory_pm` and produced the expected **2 errors** before PM code existed:
  the template was absent and `ensure_inventory_pm_visit_table` was undefined. No source
  behavior or protected database state changed during this checkpoint.
- Added the additive `InventoryPmVisit` model in `app.py` with fixed Genoray/Vieworks
  brand identity, equipment serial, exact target date, nullable schedule link, timestamps,
  and lookup indexes. The model records PM history independently while leaving existing
  Product and Shift relationships unchanged; PM table creation and routes are next.
- Added the additive PM schema-ensure helper, fixed-brand authorization/item resolver,
  April-March fiscal-year grouping, same-owner PM schedule validation, request-time
  completion/overdue derivation, schedule engineer metadata, owner-change warnings, and
  existing archive-file link serialization helpers in `app.py`. These helpers do not create
  or modify calendar schedules or copy attachment files.
- Registered the PM table ensure step in the normal SQLite compatibility-migration hook so
  a fresh or existing database receives the additive table before any PM route reads it.
  The helper remains one-time and commits pending migration writes before DDL to avoid
  SQLite request-lock contention.
- Added the shared eligible-schedule serializer for the PM link picker, including service
  type, PM task, date, client, product, and assigned-engineer metadata while keeping the
  existing Shift row read-only.
- Corrected the PM visit serializer's live linked-shift ID resolution so schedule-option
  serialization does not reference an undefined shift and deleted linked shifts return a
  null current link plus their preserved historical ID.
- Added guarded Genoray/Vieworks PM overview and equipment-detail pages plus JSON APIs for
  fiscal-year data, visit list/add/edit/delete, item detail, and eligible schedule options.
  Visit links validate service type, PM task, and current owner; one schedule may support
  multiple visits, while status/date/engineers/files are derived from the current schedule
  at request time and historical owner mismatches are flagged.
- Added the equipment-detail visits compatibility endpoint for both GET and POST so callers
  may list or add visits using the serial embedded in the brand-only URL while sharing the
  same authorization and validation path as the general visit endpoint.
- Included live item name and current owner metadata on serialized PM visits and recalculated
  overview counts after server-side filters so API consumers receive the same equipment and
  status context shown by the monitoring page.
- Expanded the isolated PM test suite with detail-URL POST/GET coverage, invalid link
  rejection, owner-change warnings, missing-schedule history retention, permitted TSR-file
  links, and Vieworks serial-edit/delete protection. These checks continue to use a temporary
  SQLite database and do not exercise the protected scheduler database.
- Corrected the service-worker history comment to identify the v160-to-v161 navigation-shell
  bump accurately while retaining the v161 PM-monitoring cache marker.
- Added focused release-contract assertions for the PM manifest entry and service-worker
  cache floor v161 so future cache changes cannot silently serve stale navigation markup.
- Fixed PM overview refresh state so changing fiscal years updates the cached payload used by
  subsequent local filters instead of rendering stale visits from the prior fiscal year.
- Added a 375px PM layout breakpoint that stacks summary fields and preserves 44px tap targets
  on narrow phones while retaining the desktop horizontal table and existing product layout.
- Added a rendered-shell regression check proving authorized navigation exposes both guarded
  PM links, standalone Genoray rows receive their brand endpoint, and Product Inventory keeps
  an empty PM base URL.
- Added the scheduled-month selector to the PM overview and aligned local/API filtering so
  April-through-March month selection works alongside fiscal year, equipment, client, and
  status filters.
- Added an API assertion for the scheduled-month filter and filtered status count in the
  disposable PM test suite.
- Bound text PM filters to input events and status/month selectors to change events so
  desktop and phone filter controls refresh predictably.
- Pinned source-level PM UI contracts for the scheduled-month control, 375px breakpoint, and
  44px narrow-phone tap target in the focused test suite.
- Verification checkpoint: PM plus Genoray and Vieworks inventory tests passed (35/35);
  Product and related coverage tests passed (11/11); calendar/service-file tests passed
  (45/45); navigation/offline/service-worker tests passed (108/108); changelog workflow and
  coverage tests passed (43 passed, 1 skipped). Isolated full discovery ran 1,175 tests with
  1,153 passed, 20 unrelated baseline failures, and 2 skips; failures were existing changelog
  marker/state, rate-limit, purchase-order setup, and staff-fixture issues outside PM scope.
- Final focused PM/inventory rerun after the month-filter, detail-alias, and shell-state fixes
  passed 36/36 tests in a fresh disposable SQLite database.
- Marked the PM plan **Executed — uncommitted** in `plans.md` with the implementation files,
  verification totals, baseline full-discovery limitations, and remaining commit/publication/
  review gates. The protected dirty database, handoff, `.claude/`, `output/`, and `tmp/` work
  remain untouched.
- Updated Genoray and Vieworks item edits to carry PM visit serial references atomically
  when a machine serial changes, and blocked item deletion while PM history exists with a
  clear conflict response. Product Inventory deletion/edit behavior remains unchanged.
- Added `templates/inventory_pm.html` with separate brand-aware overview and equipment-detail
  layouts, April-March month sections, fiscal-year/status/client/equipment filters, counts,
  print and horizontal-scroll table behavior, phone cards, visit editing, schedule linking,
  owner-mismatch warnings, and permitted attachment links. Product Inventory's existing
  presentation remains unchanged.
- Added guarded PM links to Genoray and Vieworks inventory row/card actions in
  `templates/products.html`, with a shared brand-specific detail URL and responsive action
  styling. Product Inventory rows/cards do not render a PM action.
- Added administrator-guarded Genoray PM and Vieworks PM submenu entries in
  `templates/layout.html`, with active/open state for overview aliases and equipment-detail
  paths while preserving the existing Product Inventory link and role visibility.
- Corrected the new PM fixture to flush disposable test clients before assigning foreign-key
  owner IDs to standalone inventory items; this only affects test setup and does not touch
  application data.
- Normalized the disposable PM fixture serial values to the same uppercase convention used
  by the standalone inventory APIs, keeping the serial-edit regression focused on PM history
  transfer rather than legacy mixed-case lookup behavior.
- Corrected the PM regression expectation for a reopened schedule whose 2026-04-01 target
  is already before Manila today: reopening removes Completed, then the defined derived
  lifecycle correctly reports Overdue.
- Added the published `2026-09-16-inventory-pm-monitoring` release item to
  `static/changelog/releases.json` for the administrator-facing Genoray and Vieworks PM
  monitoring pages and APIs.
- Bumped the embedded service-worker navigation cache marker from v160 to
  `v161-inventory-pm-monitoring` for the new PM navigation shell, and updated the existing
  Vieworks source contract to accept monotonic cache-version assertions.
- Began the separately authorized Recurring PM generation and schedule matching implementation.
  Preflight confirmed the approved plan, the existing uncommitted Genoray/Vieworks PM package,
  the protected dirty `scheduler.db`, handoff, `.claude/`, `output/`, and `tmp/` artifacts, and
  the restrictions against commit, push, deployment, production data, Railway changes, and
  browser/Codex UI automation. The plan is now **In progress**; recurring PM source, tests,
  release metadata, service-worker marker, and final verification remain in this work cycle.
- Extended `tests/test_inventory_pm.py` with the recurring-cadence, month-clamping, fiscal
  crossover, duplicate/atomicity, cadence-preservation, exact schedule-match, historical-link,
  migration, and both-brand contracts. The fail-first run used the disposable test database via
  `venv\\Scripts\\python.exe -m unittest tests.test_inventory_pm` and correctly exposed the
  missing cadence schema/helpers, recurring POST contract, target-date schedule filtering, and
  current UI markers; protected `scheduler.db` was not opened or changed.
- Implemented additive recurring PM storage and date helpers in `app.py`: `InventoryPmVisit`
  now stores nullable cadence for legacy rows, the table migration adds the column safely and
  creates a composite unique index when legacy duplicates do not prevent it, and deterministic
  month shifts clamp invalid destination days. New PM POST requests validate `serial_number`,
  `cadence`, and `start_date`, create Semi-Annual or Quarterly visits in one transaction, skip
  preexisting equipment/date rows, return created/skipped date lists, and leave visits unlinked.
- Updated individual PM edits to preserve cadence, restrict changes to the visit date and
  schedule link/unlink behavior, reject equipment/date collisions, validate every replacement
  against the visit's exact month and current equipment, and retain unchanged historical links.
  Schedule options now require a target date and match only same-client service PM schedules
  with a present Product whose trimmed case-insensitive name equals the standalone item name.
- Updated `templates/inventory_pm.html` with required cadence/start-date controls, live two- or
  four-date preview, created/skipped messaging, cadence display on overview/detail/phone rows,
  per-visit schedule loading, and explicit keep-current/unlink handling for historical links.
  The existing responsive, print, file-link, status, filter, and access surfaces remain in place.
- Added PM regression checks for required cadence/start-date validation, unlinked batch rows, and
  Vieworks-specific Product-name schedule matching; the focused PM suite now passes **21/21**.
- Amended the existing `2026-09-16-inventory-pm-monitoring` release description in
  `static/changelog/releases.json` to describe recurring plans and exact schedule matching, and
  bumped the embedded navigation service-worker marker from v161 to
  `v162-recurring-pm-matching`.
- Completed the approved Recurring PM generation and schedule matching plan and marked it
  **Executed — uncommitted** in `plans.md`. Final verification passed `tests.test_inventory_pm`
  **21/21**, related PM/inventory/product/layout/offline suites **103/103**, calendar/file/HR
  suites **61/61**, navigation/offline/theme suites **136/136**, and changelog workflow/
  coverage **44 passed, 1 skipped**. Isolated full discovery ran **1185 tests: 1163 passed,
  20 unrelated baseline failures, 2 skipped** (changelog manifest state, purchase-order
  setup rate limits, staff fixtures, and stale TSR cache-marker expectations). Python AST,
  Jinja, inline JavaScript, release JSON, and `git diff --check` checks passed; no browser,
  commit, push, deployment, Railway, or production database action was performed, and the
  protected dirty database, handoff, `.claude/`, `output/`, `tmp/`, and unrelated work remain
  preserved.
- Began the separately authorized Editable PM cadence with future-plan rebuilding implementation.
  Preflight confirmed the approved cadence-rebuild scope, the existing uncommitted Genoray/
  Vieworks recurring PM package, protected `scheduler.db`, handoff, `.claude/`, `output/`, and
  `tmp/` artifacts, and the restrictions against commit, push, deployment, production data,
  Railway changes, and browser/Codex UI automation. The plan is now **In progress**; additive
  plan identity, transactional rebuilding, tests, UI, release metadata, service-worker marker,
  and final verification remain in this work cycle.
- Extended `tests/test_inventory_pm.py` with fail-first contracts for shared recurring-plan
  keys, both cadence rebuild directions, edited-date anchoring, earlier-history preservation,
  later-sibling replacement, legacy no-key rows, linked-visit protection, collision rejection,
  atomic rollback, unchanged-cadence behavior, and the rebuild UI markers. The disposable
  checkpoint ran 29 tests and correctly produced 5 assertion failures plus 4 expected setup/
  contract errors against the current implementation; protected `scheduler.db` was not used.
- Added nullable indexed `InventoryPmVisit.plan_key` with additive migration support and one
  opaque key shared by every newly generated recurring batch. Individual PM PUT/PATCH requests
  now accept supported cadence changes and rebuild only the selected visit plus later siblings
  in one locked transaction, preserving earlier rows, rejecting linked selected/later visits,
  rejecting retained-date collisions, rolling back all mutations on failure, and treating
  legacy no-key rows as standalone new plan starts. Rebuild responses expose the updated row,
  regenerated visits/dates, removed future dates, and precise protection/collision details;
  unchanged-cadence edits retain the existing narrow date/link behavior.
- Updated `templates/inventory_pm.html` with an edit-mode cadence selector, selected-date
  rebuild preview, protected-link explanation, confirmation modal listing affected future dates,
  and dynamic **Update and Rebuild Plan** action while preserving existing schedule matching,
  historical-link controls, filters, responsive layout, and print behavior.
- Amended the existing inventory PM release description in `static/changelog/releases.json`
  and bumped the embedded navigation service-worker marker from v162 to
  `v163-editable-pm-cadence` so installed clients receive the editable cadence UI.
- Extended the PM additive-migration regression to verify legacy tables receive nullable
  `plan_key` alongside `cadence` and preserve both fields as null for existing rows.
- Completed the approved Editable PM cadence with future-plan rebuilding plan and marked it
  **Executed — uncommitted** in `plans.md`. Final focused PM verification passed **29/29**;
  affected inventory/Product/layout/HR/calendar-file/navigation/offline/service-worker and
  changelog modules passed **358 tests with 1 skip**. Isolated full discovery ran **1193 tests:
  1171 passed, 20 unrelated baseline failures, 2 skipped**; no PM test failed. Python AST,
  Jinja, inline JavaScript, release JSON, and `git diff --check` checks passed. No browser,
  commit, push, deployment, Railway, or production database action was performed, and the
  protected dirty `scheduler.db`, handoff, `.claude/`, `output/`, `tmp/`, and unrelated work
  remain preserved.
- Updated the top plan status to **Executed — uncommitted** after implementation and
  verification completed; no commit or publication was authorized in this cycle.
...
- Fixed Vieworks first-load SQLite self-contention in `app.py`: `ensure_vieworks_item_table()`
  ...
  local log showed `/vieworks` waiting about a minute and returning HTTP 500 on `CREATE UNIQUE
  INDEX ... database is locked`, while `/get_clients` also waited about 66 seconds. An isolated
  disposable-DB fail-first probe reproduced that index lock with a pending request writer;
  `tests/test_vieworks_inventory.py` now verifies initialization completes and preserves the
  preceding write. The focused Vieworks suite passed **10/10**, combined Genoray/Vieworks
  verification passed **24/24**, and `git diff --check` passed. The local Flask process was
  not restarted, and the live protected database was not used for testing. No live
  `scheduler.db` data, Product/Genoray behavior, Railway setting, or browser/Codex UI state
  was changed.
- Implemented the authorized standalone Vieworks Inventory package in `app.py`, including
  separate `vieworks_item` and `vieworks_bsid_counter` tables, additive schema/index setup,
  durable transactional `V-000001` six-digit-minimum allocation, nonreuse after deletion or
  edits, manual-number reservation, and case-insensitive nonblank BSID uniqueness. Vieworks
  storage remains isolated from Product Inventory, Genoray, schedules, purchase orders, and
  calibration certificates.
- Added the administrator-only `/vieworks` page plus guarded list, summary, CRUD, CSV import,
  and CSV export routes. The routes apply the verified active admin/superadmin/regional-admin
  boundary, use Genoray-compatible owner, warranty-date, contract, status, serial, duplicate,
  logging, response, and CSV conventions, ignore incoming BSIDs for new rows, preserve stored
  IDs during CSV updates, and allow nonblank admin replacement without allowing a clear.
- Extended `templates/products.html` with the Vieworks seven-column desktop/mobile mode,
  filtering, sorting, column freezing, counts, dialogs, print, CRUD, and CSV controls, using
  Vieworks-specific labels/endpoints and saved preferences. Added the guarded Inventory child
  and active-path handling in `templates/layout.html`, bumped the service worker from v159 to
  v160, and added the published Vieworks release item to `static/changelog/releases.json`.
- Added isolated `tests/test_vieworks_inventory.py` and updated the affected Genoray/sidebar
  cache contracts to use monotonic-version assertions. The fail-first Vieworks checkpoint
  correctly failed before implementation; after implementation the Vieworks suite passed 9,
  the Genoray suite passed 14, the combined related inventory/product suites passed 70, and the
  navigation/offline/service-worker/schedule/stock group passed 181. AST/Jinja/JSON, rendered
  Vieworks inline-JavaScript syntax, and `git diff --check` validation passed.
- Isolated full test discovery ran 1,163 tests with 1,142 passed, 19 unrelated baseline
  failures, and 2 skipped (the existing changelog-manifest, purchase-order rate-limit, and
  staff-fixture failures). No browser automation, production database/Railway operation,
  commit, or push was performed; protected scheduler/handoff/.claude/output/tmp and unrelated
  dirty work were preserved. Python `py_compile` could not write the protected existing
  `__pycache__`, so AST parsing was used for the source syntax check.
- Began the separately authorized Vieworks Inventory implementation after the owner’s explicit
  `approved. implement it` go-ahead. Preflight confirmed the approved plan, current uncommitted
  Genoray implementation, isolated test module, release metadata, and protected dirty artifacts;
  no production database, Railway, browser, commit, or push action is in scope.
- Added the initial isolated `tests/test_vieworks_inventory.py` fail-first checkpoint before the
  Vieworks implementation. Against the current source, the checkpoint correctly failed because
  `ensure_vieworks_item_table` was absent and the Vieworks source, navigation, release, and
  service-worker contracts were not yet present; no application or database behavior was changed
  by the checkpoint.
- Recorded the owner-submitted Vieworks Inventory package at the top of `plans.md` with status
  **Approved — awaiting go-ahead**. The executable plan specifies standalone Vieworks item and
  durable `V-000001` BSID-counter tables, admin-only page/API/CSV routes, the shared seven-
  column template mode, guarded Inventory submenu link, service-worker cache bump, release
  item, isolated fail-first and regression tests, protected-work limits, and a separate
  execution gate. This planning record did not modify application behavior or database data.
- Recorded the owner-submitted Automatic Genoray BSIDs package at the top of `plans.md` with
  status **Approved — awaiting go-ahead**. The executable plan covers a durable Genoray-only
  counter starting at `G-00001`, generation on page and CSV creation, preservation of existing
  and CSV-updated BSIDs, admin edit/duplicate/clear rules, shared-template behavior, isolated
  verification, release-record update, and protected-work limits. This planning change did
  not modify application behavior, database data, or the current uncommitted Genoray code.
- Began the separately authorized Automatic Genoray BSIDs implementation after the owner’s
  explicit `approved. implement it` go-ahead. Preflight confirmed the approved plan, current
  uncommitted Genoray implementation, isolated focused test module, release item, and protected
  dirty artifacts; no production database, Railway, browser, commit, or push action is in scope.
- Started the owner-authorized Genoray Inventory implementation. The package adds a separately
  guarded Genoray table, admin-only page/API/CSV workflows, Inventory submenu navigation,
  Product-template reuse, release metadata, a service-worker cache bump, focused isolated
  verification, and required plan/journal updates. Protected scheduler.db, handoff files,
  .claude/, output/, tmp/, production data, Railway settings, browser automation, commit, and
  push remain outside scope.
- Implemented the standalone `GenorayItem`/`genoray_item` inventory in `app.py`, including the
  active-admin permission helper, additive table/index setup, page/list/summary/CRUD routes,
  owner mapping, duplicate serial and case-insensitive BSID checks, warranty/contract status,
  atomic CSV import validation, CSV export, and activity logging. Genoray records remain outside
  Product, schedule, purchase-order, and calibration-certificate workflows.
- Adapted `templates/products.html` for Product or Genoray mode with the shared seven-column
  responsive table, mobile cards, filters, sorting, column freezing, counts, dialogs, print, and
  CSV controls. Updated `templates/layout.html` with administrator-only Inventory submenu access,
  added separate Genoray sort/freeze preferences and endpoint labels, bumped the embedded
  service-worker shell cache to v159, and added the published Genoray release entry. Final
  template whitespace was normalized after the syntax check with no behavior change.
- Added `tests/test_genoray_inventory.py`. The unchanged-HEAD fail-first checkpoint found all seven
  expected Genoray contracts absent. Focused Genoray verification passed **7/7**; related Product,
  navigation, Stock Inventory, HR Schedule Viewer, service-worker, and offline checks passed
  **110/110**. Python AST/Jinja/Node inline JavaScript/embedded service-worker/JSON and
  `git diff --check` checks passed; `py_compile` was unable to write the protected existing
  `__pycache__` path, so AST parsing was used for Python syntax.
- The isolated full suite used a disposable SQLite database and ran **1146 tests** (**1125 passed,
  19 unrelated existing Changelog/Purchase Order/staff-creation failures, 2 skipped**); no Genoray
  test failed. Changes remain uncommitted and unpublished, and protected dirty artifacts were
  preserved.
- Recorded the owner-submitted Genoray Inventory implementation package in `plans.md` with
  status **Approved — awaiting go-ahead**. The plan specifies the separate Genoray table and
  guarded page/API/CSV routes, the Inventory submenu, Product-template reuse, warranty/contract
  status, release and service-worker updates, focused and full verification, protected-worktree
  limits, and the separate execution gate. No application code, database, or system behavior
  was changed by this planning record.
...
- Implemented the Automatic Genoray BSIDs package in `app.py` with an additive durable
  `genoray_bsid_counter` table. New page and CSV records now ignore supplied BSID values and
  receive monotonic server-generated IDs beginning at `G-00001`; the counter seeds above
  existing canonical G-numbers, serializes concurrent allocations with SQLite's immediate
  writer transaction, skips occupied manual numbers, and survives item deletion and edits.
- Preserved stored BSIDs on CSV updates and historical blank/manual records. Authorized Genoray
  edits can replace a nonblank BSID with uniqueness checks, while clearing an assigned BSID is
  rejected. Product Inventory routes, records, and BSID behavior remain unchanged.
- Updated the shared `templates/products.html` dialog to disable BSID entry for new Genoray
  items, explain automatic assignment, allow existing-record replacement, and use the server
  response to update the list immediately. Extended `tests/test_genoray_inventory.py` with
  generated-sequence, CSV, preservation, edit/duplicate/clear, deletion/edit nonreuse,
  concurrency, template, and Product-isolation coverage. Updated the existing Genoray release
  description in `static/changelog/releases.json`.
- The HEAD fail-first source checkpoint confirmed the automatic-BSID markers were absent before
  implementation. Focused Genoray verification passed **14/14** and related Product tests
  passed **19/19**. AST, Jinja, extracted inline JavaScript, release JSON, and `git diff --check`
  validations passed. Isolated full discovery ran **1,154 tests: 1,133 passed, 19 unrelated
  baseline failures, and 2 skips**, limited to Changelog, Purchase Order rate-limit setup, and
  staff-creation workflows. No browser, production/Railway, service-worker, commit, push, or
...
- The owner authorized committing and pushing only the TSR signature-recovery and v146
  ...
  compatibility assertions, release metadata, and plan/change records. Protected `scheduler.db`,
  handoffs, `.claude/`, output, temporary files, and unrelated artifacts remain excluded. Existing
  implementation verification applies; staged whitespace, remote main, and Railway deployment
  metadata will be checked. No manual redeploy, database repair, or Railway-variable change is included.

- Started the separately authorized v146 TSR client-signature finalization adjustment. Save Draft
  remains signature-preserving, while successful online/revision finalization will retain the
  client signature only inside the signed PDF and remove it from completed submission JSON and
  completed local state. Offline, pending, failed, and rolled-back final saves remain retryable
  with the signature intact until confirmed success. Signature recovery is being narrowed to
  matching unfinished device drafts and genuinely pending unsent queue items; completed online TSRs,
  attachment-only server-confirmed queue items, and revision history are explicitly excluded.
  The prior v145 online/history recovery requirement is superseded in `plans.md`; protected dirty
  artifacts remain owner-owned and no database migration, production repair, browser, commit, push,
  deployment, or formal review action has occurred.

- Completed the authorized v146 lifecycle adjustment across app.py and templates/offline_tsr.html.
  Final online and corrected TSR routes validate both signatures while generating/validating the
  signed PDF, then persist only the engineer signature in submission JSON; the acknowledged/client
  image remains available to the PDF path and existing signed PDF attachment. Idempotent replay
  remains metadata-only, and rollback/failure leaves no completed submission.
- Restricted Recover available signatures to same-schedule/client unfinished IndexedDB drafts,
  account drafts, localStorage fallback, and genuinely pending unsent queue entries. Completed
  online submissions, revision history, completed queues, and queues with a server-confirmed core
  submission are excluded without online-history fetches. Pending server-draft deletion keys stop
  hydration from reintroducing a finalized client signature.
- Queue persistence now retains both signatures until the server confirms the core finalization;
  once that confirmation exists for attachment-only follow-up, the durable queue/draft payload
  strips only the client signature. Successful corrected saves reset the form after cleanup,
  while engineer profile/settings and existing signed PDFs remain unchanged.
- Added disposable external-DB route tests for successful online/revision persistence, signed-PDF
  input, idempotent replay, client-signature validation, rollback, unfinished-only recovery,
  hydration/reset safety, and queue lifecycle. Focused lifecycle/recovery checks passed **19/19**;
  related TSR/offline/resilience checks passed **103/103**. Full isolated discovery with unique
  external MEDICAL_SERVICE_TEST_DB ran **1024 tests: 1013 passed, 10 known baseline failures, and
  1 skip** (8 Purchase Order HTTP 429 setup failures and 2 Staff Creation fixture/initials
  failures). AST/Jinja/JSON/key/diff checks and targeted Node runtime checks passed; full rendered
  inline-script node syntax was not treated as a gate because existing layout entity escaping
  prevents direct node parsing.
- Bumped the embedded worker marker and exact assertions once to
  medical-service-pwa-offline-navigation-v146-tsr-signature-finalization and updated the existing
  2026-09-08 TSR release text so it no longer promises online/history recovery. No production
  signature restore/purge, database migration/repair, browser, commit, push, deployment, or
  formal review occurred; protected scheduler.db, handoffs, .claude/, output/outputs/, and tmp/
  remain owner-owned.

- Started the separately authorized TSR signature-recovery package after the prior builder stopped
  at the usage limit. The approved scope adds explicit same-schedule/client recovery choices for
  missing engineer/client signatures from matching local, queued, fallback, submitted, and
  permitted revision sources; protects surviving local signatures during server-draft hydration;
  and preserves existing requester/acknowledger, save-order, schedule-change, and corrected-TSR
  reset behavior. No source implementation, production/Railway, browser, database, commit, push,
  deployment, or formal review action has occurred yet; protected dirty artifacts remain untouched.
- Recorded the full approved recovery plan at the top of `plans.md` with numbered implementation,
  fail-first, verification, cache/release, journal, and handoff steps. Runtime checks will use a
  unique disposable external SQLite database and exclude `scheduler.db`, handoffs, `.claude/`,
  generated output, temporary files, and unrelated worktree changes.
- Added `tests/test_tsr_signature_recovery.py` and ran its required unchanged-source fail-first
  checkpoint: **9 tests: 0 passed, 9 intentional failures, 0 errors, 0 skips**. The expected red
  contracts cover explicit recovery controls, same-schedule/client filtering, local/queue/fallback
  and online revision traversal, missing-slot preservation, stale/failure reporting, hydration
  merging, and v145 delivery records; no application behavior changed in this checkpoint.
- Implemented explicit `Recover available signatures` controls in `templates/offline_tsr.html`.
  Draft cards now expose recovery only for missing slots; the modal searches matching IndexedDB
  drafts, queued TSRs, localStorage fallback, the saved online TSR, and bounded permitted parent
  revisions. The engineer selects the missing engineer/client signature slot(s) and one labeled
  source; existing values remain untouched, mismatched schedule/client candidates are excluded,
  stale draft context is rejected, and save success, localStorage fallback, or total storage
  failure is reported truthfully. Corrected TSR acknowledgement reset and explicit historical-source
  messaging remain in place.
- Updated `mergeServerStandaloneTSRDrafts()` to preserve matching surviving local signature fields
  when an account payload omits them, while refusing mismatched schedule/client or corrected
  revision merges and retaining the existing attachment and timestamp/save-chain behavior.
- Added controlled runtime coverage for partial selection, local queue/fallback and online revision
  traversal, mismatch rejection, and non-revision hydration merging. Final focused checks passed:
  **11/11**; related TSR/offline checks passed **95/95**; exact-cache assertion checks passed
  **218/218**. The isolated full suite ran **1014 tests** with **1003 passed, 10 known baseline
  failures, and 1 skip** (eight Purchase Order HTTP 429 setup failures and two Staff Creation
  fixture/initials failures). Python, Jinja, JavaScript, release-manifest, and diff checks passed.
- Bumped the embedded worker marker to
  `medical-service-pwa-offline-navigation-v145-tsr-signature-recovery`, updated every exact cache
  assertion, and published `2026-09-08-tsr-signature-recovery` in `static/changelog/releases.json`.
  No commit, push, Railway/production action, browser/Codex UI action, or protected-artifact staging
  occurred; `scheduler.db`, handoffs, `.claude/`, `output/`, `outputs/`, and `tmp/` remain owner-owned.
...
- Added approval-scoped Calibration Report metadata to the Calibration Certificate serializer in `app.py`, resolving only the DOCX `ShiftFile` marked as generated by the same TSR submission and matching shift. The serialized link carries the approval id and is authorized only for the related requester, assigned approver, or authorized administrator; generic TSR archive visibility remains unchanged.
- Updated the Calibration Certificate detail modal in `templates/approvals.html` with an escaped `Download Calibration Report (DOCX)` new-tab link and an unavailable state when the exact report metadata is absent. The existing certificate preview, return remarks, and Approve/Return controls remain intact; no service-worker bump was needed because the archive download path is already network-only.
- Added approval workflow and Approval Center wording regression coverage for serializer output, exact-file download authorization, successful DOCX delivery, unauthorized/mismatched-file denial, and safe link attributes. The fail-first controls ran against the unchanged app with **3 tests: 2 failures and 1 error**, confirming the missing implementation before source changes.
- Added the approver-facing Calibration Report link to the 2026-09-02 release manifest and recorded the isolated execution plan in `plans.md`. The feature implementation is committed as `c52d742` in the clean feature worktree; the implementation and documentation closeout were published to `origin/main` after the remote-head recheck.
- Post-fix verification passed the focused Calibration Certificate workflow plus Approval Center wording suite **21/21**, and the related workflow, wording, and TSR archive suite **39/39**. Timeline TSR download coverage passed **7/7** with the test-only in-process bypass for the pre-existing reimbursement-receipt DDL lock; no project source was changed for that bypass.
