...
class TsrAssignedEngineersLayoutTests(unittest.TestCase):
    ...
    def test_single_day_team_uses_footer_not_body_coverage(self):
        ...
        self.assertIn('if(!shouldRenderTSRScheduleCoverage(data)) return', self.source)
        self.assertIn('Other Assigned Engineer(s):', self.source)
        self.assertIn('const showCoverage = shouldRenderTSRScheduleCoverage(data);', self.source)
    ...
    def test_backend_does_not_add_footer_for_multi_day(self):
        rows = [
        ...
        self.assertEqual(get_tsr_other_assigned_engineer_names(rows, 'Jonamar Paunil'), [])
    ...
    def test_archive_preview_is_fingerprint_and_no_cache_aware(self):
        ...
        self.assertIn("medical-service-pwa-offline-navigation-v20-tsr-team-footer", app_source)
