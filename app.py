...
def build_schedule_activity_context_from_payload(payload, shift=None):
    """Return client/product labels for schedule activity logs.

    Robust activity-log source:
    - reads multiple frontend key variants
    - resolves Client/Product directly from DB instead of relying on relationship hydration
    - falls back to already-loaded shift relationship when available
    """
    payload = payload or {}

    client_id = (
        clean_int(payload.get('client_id')) or
        clean_int(payload.get('client')) or
        clean_int(payload.get('selected_client_id')) or
        clean_int(payload.get('selectedClientId')) or
        clean_int(payload.get('clientId'))
    )
    ...
    client_label = (
        ...
        clean_str(payload.get('selected_client_name')) or
        clean_str(payload.get('selectedClientName')) or
        clean_str(payload.get('customer_name')) or
        clean_str(payload.get('hospital_name')) or
        ''
    )

    product_label = (
        clean_str(payload.get('product_name')) or
        clean_str(payload.get('selected_product_name')) or
        clean_str(payload.get('selectedProductName')) or
        clean_str(payload.get('equipment_name')) or
        clean_str(payload.get('equipment_model')) or
        ''
    )

    client_rec = db.session.get(Client, client_id) if client_id else None
    ...
    if product_rec:
        ...
        if product_name and product_serial:
            product_label = f"{product_name} ({product_serial})"
        else:
...
# --- ONE-TIME ACTIVITY LOG BACKFILL: schedule client/product labels ---

def normalize_activity_schedule_title(value):
    value = clean_str(value) or ''
    value = re.sub(r'\s+', ' ', value).strip().lower()
    return value


def parse_added_calendar_schedule_log(action):
    """Parse old add-schedule logs.

    Supported old shape:
    Added calendar schedule: <task> on <YYYY-MM-DD>[ to <YYYY-MM-DD>] for <engineers>
    """
    raw = action or ''
    match = re.search(
        r'Added calendar schedule:\s*(?P<title>.*?)\s+on\s+(?P<start>\d{4}-\d{2}-\d{2})(?:\s+to\s+(?P<end>\d{4}-\d{2}-\d{2}))?',
        raw,
        flags=re.I
    )
    if not match:
    ...
    end_date = parse_date(match.group('end') or match.group('start'))
    title = clean_str(match.group('title')) or ''
...
def find_shift_for_activity_log_backfill(parsed_log):
    """Find one best matching Shift for an old schedule activity log."""
    if not parsed_log:
        return None

    start_dt, end_dt = build_shift_datetime_bounds(parsed_log['start_date'], parsed_log['end_date'])
    title_norm = normalize_activity_schedule_title(parsed_log.get('title'))
    ...
    exact_title_matches = [
        ...
        if normalize_activity_schedule_title(shift.title) == title_norm
    ]

    if exact_title_matches:
        # Prefer rows with client info because the backfill goal is client visibility.
        exact_title_matches.sort(key=lambda shift: (
            0 if shift.client_id else 1,
            shift.start_time or datetime.max,
            shift.id
        ))
    ...
    soft_matches = [
        shift for shift in candidates
        if title_norm and (
            title_norm in normalize_activity_schedule_title(shift.title) or
            normalize_activity_schedule_title(shift.title) in title_norm
        )
    ]

    if soft_matches:
        soft_matches.sort(key=lambda shift: (
            0 if shift.client_id else 1,
            shift.start_time or datetime.max,
            shift.id
        ))
...
def build_activity_backfill_labels_from_shift(shift):
    """Build Client/Product/Engineer labels from a matched Shift."""
    if not shift:
        return {}

    client_label = shift.client.name if shift.client else ''
    product_label = ''
    if shift.product:
        product_name = clean_str(shift.product.name) or ''
        product_serial = clean_str(shift.product.serial_number) or clean_str(shift.product_id) or ''
        product_label = f"{product_name} ({product_serial})" if product_name and product_serial else (product_name or product_serial)
    elif shift.product_id:
        product_label = shift.product_id

    engineer_records = get_shift_engineer_records(shift)
    engineer_names = [engineer.name for engineer in engineer_records if engineer and engineer.name]
...
@app.route('/admin/backfill-schedule-activity-clients', methods=['POST'])
@login_required
def admin_backfill_schedule_activity_clients():
    """One-time superadmin migration for old ActivityLog schedule-add rows.

    Safe behavior:
    - only touches logs beginning with "Added calendar schedule:"
    - skips logs that already contain "| Client:"
    - matches by title + date range against existing Shift rows
    - leaves uncertain/unmatched logs unchanged
    """
    if not is_superadmin_user():
        return denied('Only superadmins can run activity log backfills.')

    dry_run = parse_bool_flag(request.args.get('dry_run'), default=False)
    limit = clean_int(request.args.get('limit')) or 5000

    old_logs = (
        ActivityLog.query
        .filter(ActivityLog.action.ilike('Added calendar schedule:%'))
        .filter(~ActivityLog.action.ilike('%| Client:%'))
    ...
    for log in old_logs:
        parsed = parse_added_calendar_schedule_log(log.action)
        if not parsed:
            skipped_unmatched += 1
            continue

        matched_shift = find_shift_for_activity_log_backfill(parsed)
        if not matched_shift:
            skipped_unmatched += 1
            continue

        matched += 1
        labels = build_activity_backfill_labels_from_shift(matched_shift)

        if not labels.get('client_label'):
            skipped_no_client += 1
            continue

        new_action = log.action

        if labels.get('client_label') and '| Client:' not in new_action:
            new_action += f" | Client: {labels['client_label']}"

        if labels.get('product_label') and '| Product:' not in new_action:
            new_action += f" | Product: {labels['product_label']}"

        # Add engineers only if the old log did not already contain engineer wording.
        if labels.get('engineer_names') and 'Engineer(s):' not in new_action and ' for ' not in new_action:
            new_action += f" | Engineer(s): {', '.join(labels['engineer_names'])}"

        if new_action != log.action:
            updated += 1
            if len(samples) < 10:
                samples.append({
                    'id': log.id,
    ...
    if not dry_run and updated:
        db.session.add(ActivityLog(
            user=(getattr(current_user, 'username', '') or 'Superadmin').capitalize(),
            action=f"Backfilled schedule activity logs with client/product labels: {updated} updated"
        ))
        db.session.commit()
    elif not dry_run:
        db.session.commit()
    ...
    return jsonify({
        'status': 'success',
        'dry_run': dry_run,
        'scanned': len(old_logs),
        'matched': matched,
        'updated': updated,
        'skipped_unmatched': skipped_unmatched,
        'skipped_no_client': skipped_no_client,
        'samples': samples,
        'message': 'Dry run only. No logs changed.' if dry_run else f'Backfill complete. {updated} old log(s) updated.'
    })
...
def add_shift():
    ...
    # Resolve directly from payload/DB so the log is correct even before relationship refresh/commit.
    schedule_activity_context = build_schedule_activity_context_from_payload(payload, first_shift)
    client_label = schedule_activity_context.get('client_label') or ''
    product_label = schedule_activity_context.get('product_label') or ''
